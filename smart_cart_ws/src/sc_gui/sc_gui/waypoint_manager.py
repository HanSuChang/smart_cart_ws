...
class InteractiveMapPanel(QFrame):
    ...
    def __init__(self, yaml_path):
        ...
        title_label = QLabel(" RADAR & WAYPOINT EDITOR (좌클릭: 추가 / 우클릭: 삭제)")
    ...
    def load_map_yaml(self, yaml_path):
        """💡 지도가 maps 폴더에 있어도 이미지를 완벽하게 찾아내는 로직"""
        ...
        try:
            ...
            # 💡 핵심: YAML에 적힌 경로가 절대경로든 상대경로든 무시하고
            # 현재 YAML 파일과 같은 폴더에서 이미지 파일명을 찾아 절대경로를 생성합니다.
            ...
            if os.path.exists(img_path):
                ...
                self.map_label.set_original_size(self.original_pixmap.width(), self.original_pixmap.height())
    ...
    def handle_map_click(self, px, py):
        ...
        try:
            ...
            items = ["A구역", "B구역", "주차장(RETURN)"]
            name, ok = QInputDialog.getItem(self, "웨이포인트 저장", "라벨 선택:", items, 0, False)
            if ok and name:
    ...
    def handle_map_right_click(self, px, py):
        if not self.original_pixmap or not self.wp_db.data: return
        try:
            real_x, real_y = self._px_to_real(px, py)
    ...
    def redraw_map(self):
        ...
        painter.end()
        self.map_label.setPixmap(canvas.scaled(self.map_label.size(), Qt.AspectRatioMode.KeepAspectRatio,
